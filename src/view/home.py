...
class Home:
    ...
    def treeview(self):
        ...
        self.treeview.column("id",width=0,minwidth=0)
        self.treeview.column("sku",width=140,minwidth=140,anchor=CENTER)
        self.treeview.column("product_name",width=550,minwidth=550)
    ...
    def search_filter(self):
        ...
        ctg_search['values'] = [x for x in query_ctg().values()] #NOTE single line loop for get values in dict
    ...
    def current_ctg(event):
        ...
        get_id = [x for x in query_ctg().items()][current]
        # get_nameCtg = currentCtg.split()
        # catgEntry.insert(0,get_nameCtg[1])       
        catgEntry.insert(0,currentCtg)  
    ...
    def category_management(self):
        ...
        def add_ctg(name):
            if not name:
                ...
                return
            if name == "Uncategory":
                messagebox.showwarning("iStock","You can't edit this category")
                catgEntry.delete(0,END)   
                catgDropdown.set("")
                return
            else:
        ...
        def delete_ctg(name):
            def uncategory():
                conn = connect_sqLite()
                curs = conn.cursor()
                curs.execute("SELECT * FROM products WHERE category = ?",[name])
                ctg_check = curs.fetchall()
                conn.close()
                if not ctg_check:
                    pass
                else :
                    conn = connect_sqLite()
                    curs = conn.cursor()
                    curs.execute("UPDATE products SET category = ? WHERE category = ?",["Uncategory",name])
                    conn.commit()
                    conn.close
                retrieve_data()              
            if not name:
            ...
            if name == "Uncategory":
                ...
                conn = connect_sqLite()
                curs = conn.cursor()
                curs.execute("DELETE FROM products_category WHERE category_id = ?",[get_id[0]])
                conn.commit()
                conn.close()        
